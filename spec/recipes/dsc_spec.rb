...
describe 'powershell::dsc' do
  ...
  context 'When listener is enabled' do
    before do
      get_listener_cmd = 'powershell.exe winrm get winrm/config/listener?Address=*+Transport=HTTP'

      shell_obj = instance_double('Mixlib::ShellOut')
      allow(Mixlib::ShellOut).to receive(:new).with(get_listener_cmd).and_return(shell_obj)
